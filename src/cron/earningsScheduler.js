...
async function notifyEarningsPreAlarm(date, stock_id, symbol) {
  ...
  const urls = [
    "http://localhost:4000/api/notifications/earnings/prealarm"
  ];
  for (const url of urls) {
    try {
      console.log("알림 테스트 파라미터", { date, stock_id, symbol });
      await axios.post(url, { date, stock_id, symbol }, {
        headers: {
      ...
      if (err.response) {
        console.error(`${url} 실적 하루 전 알림 요청 실패: [${err.response.status}] ${err.response.statusText}`);
      } else if (err.request) {
        console.error(`${url} 실적 하루 전 알림 요청 실패: No response from server`);
      } else {
...
export async function notifyEarningsSummaryUpload(symbol, date) {
  ...
  try {
    ...
    const urls = [
      "http://localhost:4000/api/notifications/earnings/summary"
    ];
    for (const url of urls) {
      try {
        console.log("axios.post 직전", { symbol, date, msg });
        await axios.post(url, { symbol, date, message: msg }, {
          headers: {
            "Content-Type": "application/json"
          }
        });
        ...
        if (err.response) {
          console.error(`${url} 실적 요약+업로드 알림 요청 실패: [${err.response.status}] ${err.response.statusText}`);
        } else if (err.request) {
          console.error(`${url} 실적 요약+업로드 알림 요청 실패: No response from server`);
        } else {
      ...
      for (let m = 0; m < 60; m += 34) {
        cron.schedule(`${m} ${hour} * * *`, async () => {
          ...
          for (const e of pollingSet) {
            ...
            console.log(`[DEBUG] getSymbolByStockId 결과: stock_id=${e}, symbol=${symbol}`);
            if (!symbol) {
              console.error(`[runEarningsScheduler] symbol이 없습니다! stock_id=${e}`);
              continue; // 다음 루프로 넘어감
            }
            console.log(`[runEarningsScheduler] getSymbolByStockId 결과: stock_id=${e}, symbol=${symbol}`);
            const today = dayjs().format("YYYY-MM-DD");
            console.log(`[runEarningsScheduler] runPollingJob 호출: stock_id=${e}, label=${label}`);
            const result = await runPollingJob(e, label);
            console.log(`[runEarningsScheduler] runPollingJob 결과: stock_id=${e}, result=${result}`);
            if (result) {
              pollingSet.delete(e);
            }
