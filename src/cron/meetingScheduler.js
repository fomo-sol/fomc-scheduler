...
async function getFomcEventTime(type, date) {
  ...
  try {
    let query, timeField;
    if (type === "statement") {
      query = "SELECT fed_start_time FROM pdafomo.fomc_rate_decisions WHERE fed_release_date = ?";
      timeField = "fed_start_time";
    } else if (type === "minutes") {
      query = "SELECT fomc_start_time FROM pdafomo.fomc_minutes WHERE fomc_release_date = ?";
    ...
    if (row && row[timeField]) {
      // 한국 시간으로 변환
      return dayjs.tz(row[timeField], "Asia/Seoul").format("YYYY-MM-DD HH:mm:ss");
    }
    return null;
  } finally {
    conn.release();
...
async function notifyFomcPreAlarm(date, type, state) {
  ...
  for (const url of urls) {
    try {
      ...
      if (err.response) {
        // 서버가 응답한 경우
        console.error(`${url} FOMC 미리 알림 요청 실패: [${err.response.status}] ${err.response.statusText}`);
      } else if (err.request) {
        // 요청은 갔으나 응답이 없는 경우
        console.error(`${url} FOMC 미리 알림 요청 실패: No response from server`);
      } else {
        // 기타 에러
...
async function notifyFomcUploadAlarm(date, type) {
  ...
  for (const url of urls) {
    try {
      ...
      if (err.response) {
        console.error(`${url} FOMC 업로드 알림 요청 실패: [${err.response.status}] ${err.response.statusText}`);
      } else if (err.request) {
        console.error(`${url} FOMC 업로드 알림 요청 실패: No response from server`);
      } else {
...
export async function runFomcPreAlarmScheduler() {
  ...
  for (const meeting of meetings) {
    const statementDay = dayjs(meeting.statementDate).format("YYYY-MM-DD");
    const oneWeekBefore = dayjs(meeting.statementDate).subtract(7, "day").format("YYYY-MM-DD");
    const oneDayBefore = dayjs(meeting.statementDate).subtract(1, "day").format("YYYY-MM-DD");

    console.log(`🗓️ 금리 결정: ${statementDay}, D-7: ${oneWeekBefore}, D-1: ${oneDayBefore}`);

    if (today === oneWeekBefore) {
      console.log(`🚨 [${statementDay}] FOMC D-7 알림 전송`);
      console.log(`📦 payload → date: ${statementDay}, type: statement, state: one_week_before`);
    ...
    if (today === oneDayBefore) {
      ...
      console.log(`📦 payload → date: ${statementDay}, type: statement, state: one_day_before`);
      await notifyFomcPreAlarm(statementDay, "statement", "one_day_before");
    }

    const minutesDay = dayjs(meeting.minutesDate).format("YYYY-MM-DD");
    const minutesOneWeekBefore = dayjs(meeting.minutesDate).subtract(7, "day").format("YYYY-MM-DD");
    const minutesOneDayBefore = dayjs(meeting.minutesDate).subtract(1, "day").format("YYYY-MM-DD");

    console.log(`🗓️ 의사록: ${minutesDay}, D-7: ${minutesOneWeekBefore}, D-1: ${minutesOneDayBefore}`);

    if (today === minutesOneWeekBefore) {
      console.log(`🚨 [${minutesDay}] FOMC minutes D-7 알림 전송`);
      console.log(`📦 payload → date: ${minutesDay}, type: minutes, state: one_week_before`);
    ...
    if (today === minutesOneDayBefore) {
      ...
      console.log(`📦 payload → date: ${minutesDay}, type: minutes, state: one_day_before`);
